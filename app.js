...
const INTERNAL_PILOT_FEEDBACK_ENDPOINT=atob('aHR0cHM6Ly9zY3JpcHQuZ29vZ2xlLmNvbS9tYWNyb3Mvcy9BS2Z5Y2J3MUZETkFGMVU2REg4Z29MSWtjaWo1VU83OFE0ZUcxanVnYm5hTV9aVS1pZE1MWWtSQjhNNlFiNy10NXFKRlV5NmcvZXhlYw==');
...
function inductionMedicationRecords(){return (state.drugAdministrations||[]).filter(d=>!d.voidedAt&&d.source==='OR Induction')}
...
function renderOrPrimaryFlow(){
  ...
  if(phase==='setup'){t='Start induction';h='กดครั้งเดียวเพื่อ timestamp Induction + เริ่มจับเวลา • ปริมาณยาค่อยลงย้อนหลังได้หลัง airway stable';label='▶ Start induction';action='start-induction';}
    ...
    drug.hidden=false;drug.textContent=pendingInduction?'💉 Induction meds pending':'💉 Medications';drug.dataset.mode=pendingInduction?'induction':'generic';
  ...
  else if(phase==='emergence'){const intubated=airwayRecorded();t='Emergence';h=intubated?'กดเมื่อถอดท่อแล้ว ระบบจะสร้าง Recovery handoff และเข้า Recovery':'ไม่มี airway/intubation record — สามารถเริ่ม Recovery ได้โดยตรง';label=intubated?'🫁 Extubation → Recovery':'→ Begin Recovery';action=intubated?'extubation':'recovery';drug.hidden=false;drug.textContent=pendingInduction?'💉 Induction meds pending':'💉 Medications';drug.dataset.mode=pendingInduction?'induction':'generic';}
...
function renderEndCase(){
  ...
  const ready=!!state.recoveryCompletedAt&&signoffReady&&!(state.alertEpisodes||[]).some(a=>!a.resolvedAt)&&activeComplications===0&&['endConfirmRecovery','endConfirmRecord','endConfirmDrugs','endConfirmPdf'].every(id=>!!$(id)?.checked);
...
function renderCaseDrugPlan(){
  ...
  listEl.innerHTML=items.map((d,i)=>{const r=casePlanCalc(d,w),vol=Number.isFinite(r.ml)&&r.ml>0?`${fmtVol(r.ml)} mL`:'Dose/concentration incomplete';return `<article class="case-drug-plan-item ${d.standby?'standby':''}"><div class="case-drug-plan-main"><span class="case-drug-phase">${escapeHtml(caseDrugPlanPhaseLabel(d.phase))}</span><b>${escapeHtml(d.name)}</b><small>${escapeHtml([d.dose?`${d.dose} ${d.mode==='mcgkg'?'μg/kg':d.mode==='mgkg'?'mg/kg':d.mode==='mlkg'?'mL/kg':d.mode==='bwdiv'?'BW ÷ factor':''}`:'',d.conc?`${d.conc} ${d.concUnit||''}`:'',d.route].filter(Boolean).join(' • ')||'Manual / verify preparation')}</small></div><div class="case-drug-plan-volume"><b>${escapeHtml(vol)}</b><small>${d.standby?'STANDBY':'planned reference'}</small></div>${locked?'':`<button type="button" class="case-drug-plan-remove" data-plan-index="${i}" aria-label="Remove ${escapeHtml(d.name)}">×</button>`}</article>`}).join('');
...
function reviewCaseDrugPlan(){if(caseDrugPlanLocked())return;const by=$('anesthetist')?.value.trim()||prompt('Reviewed by','')||'';if(!by.trim()){toast('ระบุผู้ทบทวนแผนยา');return}state.caseDrugPlanReviewedAt=Date.now();state.caseDrugPlanReviewedBy=by.trim();addAudit('CASE_DRUG_PLAN_REVIEWED',`${(state.caseDrugPlan||[]).length} planned medication(s)`,by.trim());save();renderCaseDrugPlan();toast('✓ Case Drug Plan saved / reviewed')}
...
async function sendPilotFeedbackReport(report){const endpoint=INTERNAL_PILOT_FEEDBACK_ENDPOINT;if(!endpoint)throw new Error('Feedback service is unavailable');if(navigator.onLine===false)throw new Error('Device is offline');const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),12000);try{const res=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(report),signal:ctl.signal,cache:'no-store',credentials:'omit',redirect:'follow'});if(!res.ok)throw new Error(`HTTP ${res.status}`);return true}finally{clearTimeout(timer)}}
async function queuePilotFeedback(localOnly=false){const report=await makePilotFeedbackReport(localOnly);if(!report)return;let q=getPilotFeedbackQueue();q.push(report);setPilotFeedbackQueue(q);renderPilotFeedbackStatus();if(localOnly){toast(`✓ Feedback saved locally • ${report.reportId}`);closePilotFeedbackDialog();resetPilotFeedbackForm();return}try{await sendPilotFeedbackReport(report);q=getPilotFeedbackQueue();const item=q.find(x=>x.reportId===report.reportId);if(item){item.status='sent';item.sentAt=Date.now();item.attempts=(item.attempts||0)+1;item.lastError='';setPilotFeedbackQueue(q)}toast(`✓ Feedback submitted • ${report.reportId}`)}catch(e){q=getPilotFeedbackQueue();const item=q.find(x=>x.reportId===report.reportId);if(item){item.status='pending';item.attempts=(item.attempts||0)+1;item.lastError=e?.message||String(e);setPilotFeedbackQueue(q)}toast(`Saved locally • pending sync • ${report.reportId}`)}renderPilotFeedbackStatus();closePilotFeedbackDialog();resetPilotFeedbackForm()}
...
async function retryPendingPilotFeedback({silent=false}={}){const endpoint=INTERNAL_PILOT_FEEDBACK_ENDPOINT;if(!endpoint||navigator.onLine===false){if(!silent)toast(endpoint?'Offline — reports remain queued':'Feedback service unavailable');renderPilotFeedbackStatus();return}let q=getPilotFeedbackQueue(),changed=false,sent=0;for(const item of q.filter(x=>x.status==='pending'||x.status==='failed').slice(0,10)){try{await sendPilotFeedbackReport(item);item.status='sent';item.sentAt=Date.now();item.attempts=(item.attempts||0)+1;item.lastError='';sent++;changed=true}catch(e){item.status='failed';item.attempts=(item.attempts||0)+1;item.lastError=e?.message||String(e);changed=true}}if(changed)setPilotFeedbackQueue(q);renderPilotFeedbackStatus();if(!silent)toast(sent?`✓ Synced ${sent} feedback report${sent===1?'':'s'}`:'No report synced • pending items kept locally')}
